...
async function startServer() {
  ...
  server.on('error', (err: NodeJS.ErrnoException) => {
    if (err.code === 'EADDRINUSE') {
      console.error(`\n[IP3 Platform] Error: Port ${PORT} is already in use.`);
      console.error(`Please stop the process using port ${PORT} or specify a different port (e.g. $env:PORT=3001; npm run dev)\n`);
    } else {
